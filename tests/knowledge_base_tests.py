...
def element_isotopic_abundance_test():
    for element, isotope_atomic_mass_abundance_list in\
            pyqms.knowledge_base.isotopic_distributions.items():
        yield check_abundance_sum, isotope_atomic_mass_abundance_list

def check_abundance_sum(isotopic_distribution_list):
    sum_of_abundances = sum(
        [abun for mass, abun in isotopic_distribution_list]
    )
