...
class TestResults( unittest.TestCase ):
    ...
    def parse_and_filter_test(self):
        ...
        for test_dict in TESTS:
            for key in self.results._parse_and_filter(
                    **test_dict['input']
                ):
    ...
    def extract_results_test(self):
        ...
        for test_dict in TESTS:
            for key, n, entry in self.results.extract_results(
                    **test_dict['input']
                ):
                print(key, entry)
                assert key.formula == test_dict['output']['formula']
    ...
    def max_score_test(self):
        ...
        max_score[3] = entry
        '''
        max_score_tuple = self.results.max_score(
            molecules=['DDSPDLPK']
        )
        assert max_score_tuple[0] == 1  # score
        assert max_score_tuple[3].scaling_factor == 100  # intensity

        assert self.results.max_score(
            molecules=['_DDSPDLPK_']
        ) == [0, None, None, None]
        return
    ...
    def max_i_amount_test(self):
        ...
        max_I_dict = self.results.determine_max_itensity(
            obj_for_calc_amount
        )
    ...
    def rpy2_import_test(self):
        ...
        if rpy2_present:
            ...
            for key in plot_result_class._parse_and_filter(
                    molecules = ['DDSPDLPK']
                ):
                # plot 3D
                ...
                max_score_tuple = plot_result_class.max_score(
                    molecules=['DDSPDLPK']
                )
                ...
                plot_result_class._determine_measured_error(
                    ...
                    filename        = os.path.join(
                        'tests',
                        'data',
                        'mz_and_intensity_error_density_plot.pdf'
                    ),
                    plot            = True
                )
        ...
        for pos, file_extension in enumerate(['xlsx', 'csv', 'extension_unknown']):
            ...
            if pos == 2:
                ...
                if file_extension == 'csv':
                    ...
                    read_in_line_dicts = read_xlsx_file(
                        quant_summary_file
                    )
    ...
    def label_percentile_test(self):
        list_of_xzy_dicts = self.results.determine_label_efficiency(
            element = 'N'
        )
    ...
    def group_14N_15N_pairs_test(self):
        ...
        for key, value in zip(keys, values):
            tmp_metabolic_labeling_class.add(
                key,
                value
            )
        all_key_pairs = []
    ...
    def window_definition_and_curator_test(self):
        ...
        for test_dict in TESTS:
            output = self.results.curate_rt_windows(
                **test_dict['input']
            )
            for peptide in output.keys():
                assert output[peptide]['rt_window'] == test_dict['output'][peptide]['rt_window']
