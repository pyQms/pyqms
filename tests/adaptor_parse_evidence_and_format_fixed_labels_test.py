...
from pyqms.adaptors import _parse_evidence_and_format_fixed_labels as _parse_evidence_and_format_fixed_labels
import os
...
def adaptor_check( test_dict ):
    ...
    for key in test_dict['output'].keys():
        if key == 'fixed_labels':
            assert sorted(test_dict['output'][key].keys()) == sorted(result_dict[key].keys())
